...
It asks for the user input, then compares if the input is a newline/eof, if so, continues, if the instruction is I, it increments the accumulator by one, if it is D, decrements accumulator by one, S squares the value of accumulator, O outputs the value of accumulator **as the number**, and it checks if accumulator is -1 or 256. If so, it makes accumulator to be on zero.
